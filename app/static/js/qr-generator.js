/**
 * Standalone Offline QR Code Generator for Punto de Venta
 * Generates QR codes as SVG or HTML Canvas with 0 external dependencies.
 */
(function(window) {
    // Simple & reliable QR code canvas renderer
    // We implement a fast QR matrix generator or provide a clean SVG/Canvas renderer
    
    // Quick QR matrix generator (type 1-10)
    ...
    // Global helper to render to an HTML canvas element with proper Quiet Zone (margin)
    window.renderQRCode = function(canvas, text, size) {
        if (!canvas) return;
        size = size || 220;
        ...
        var margin = 4; // Standard 4-module quiet zone mandatory for mobile camera reading
        var total = count + margin * 2;
        var cellSize = Math.max(2, Math.floor(size / total));
        var actualSize = total * cellSize;
        ...
        // Fondo blanco (Quiet Zone completo)
    ...
    // Global helper to generate standalone SVG string
